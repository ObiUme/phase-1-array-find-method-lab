...
function superbowlWin(record) {
    const winYear = record.find(obj => obj.result === `W`); // utilizes find() to return a `true` statement 
    if (winYear) { // sets variable that captures find() (boolean) into a conditional
        return winYear.year; // // winYear pulls from the record.find so you are able to tag on dot anotation to the container winYear
    }
    else {
        return undefined // did not know undefined was 
    }
}
